 ...
 */
public record TencentResponseParse<T>(Class<T> clazz) implements ResponseParse<T> {
    ...
    public T parse(String body) throws ClientException {
        ...
        try {
            Type type = new TypeToken<TencentResponseJsonModel<TencentSmsApiErrResponse>>() {
            }.getType();
